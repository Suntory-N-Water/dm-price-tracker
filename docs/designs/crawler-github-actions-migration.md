...
- 採用内容:
  - 日本の送信元IPを直ちに必須要件とはしない。
  - 標準GitHub-hosted runnerと日本環境から同じメルカリ検索条件へアクセスし、取得結果を比較してから要件を確定する。
  - 比較結果に問題がなければ、標準GitHub-hosted runnerを採用する。
  - 地域によるリダイレクト、アクセス拒否、検索結果の実質的な差異がある場合は、日本からの送信元IPを確保できる実行環境を再検討する。
- 理由: 送信元IPが取得結果へ影響するかを推測で決めず、実際の挙動を根拠に必要な実行環境を選ぶため。
...
- 標準GitHub-hosted runnerと日本環境から同一条件でアクセスし、次を比較する。
  - HTTPステータスとリダイレクト先
  - 商品件数、商品URL、タイトル、価格
  - アクセス拒否や地域向け表示の有無
- 実質的な差異がなければ、送信元IPの地域は要件に含めない。
- 実質的な差異があれば、日本の送信元IPを保証できるrunnerを再検討する。
