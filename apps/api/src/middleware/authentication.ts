...
import { registerUser } from '#api/external/repository/user-repository';
...
export function authentication(
  ...
  return async (context, next) => {
    ...
    const localEmail =
      localAuthentication?.email ?? context.env.LOCAL_AUTH_EMAIL;
    const localIsAdmin =
      localAuthentication?.isAdmin ??
      (context.env.LOCAL_AUTH_IS_ADMIN === undefined
        ? undefined
        : context.env.LOCAL_AUTH_IS_ADMIN === 'true');

    let email: string;
    if (token === undefined) {
      if (localEmail === undefined || localIsAdmin === undefined) {
        return context.json({ error: '認証が必要です' }, 401);
      }
      if (isAdminRoute && !localIsAdmin) {
        return context.json({ error: '管理者権限が必要です' }, 403);
      }
      email = v.parse(emailSchema, localEmail);
    } else {
